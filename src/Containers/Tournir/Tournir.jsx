...
import { footballEvents, footballCountry, footballTeams } from "../../store/slices/football";
...
const Tournir = ({ type }) => {
  ...
  const { footballLeague, footballCountryes, footballTeam } = useSelector(
  ...
  return (
    <div className='content'>
      ...
      <Row gutter={[10, 10]} className={s.blockCenter}>
        <Col span={18}>
          <Row gutter={10}>
            {Object.keys(footballTeam).map((item) => {

              return (
                <Col span={12} xs={24} lg={12} key={item}>
                  <Link to='/turnir'>
                    <TourCard item={footballTeam[item]}  />
                  </Link>
                </Col>
              );
            })}
